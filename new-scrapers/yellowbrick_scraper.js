...
(async () => {
    ...
    try {
        ...
        console.log('Error getting unfinished race data', err);
        process.exit();
    }
    ...
    for (const currentCode of codes) {
        try {
            ...
            const forceScrapeRaceData = forceScrapeRacesMap[raceCode];
            ...
            if (forceScrapeRaceData) {
                if (setup.data.start > now / 1000) {
                    // if start time is in the future set it today
                    race.start = now / 1000;
                    race.stop = now / 1000;
                } else {
                    race.stop = forceScrapeRaceData.approx_end_time_ms / 1000;
                }
            } else if (
                setup.data.start > now / 1000 ||
                setup.data.stop === null ||
                setup.data.stop > now / 1000
            ) {
